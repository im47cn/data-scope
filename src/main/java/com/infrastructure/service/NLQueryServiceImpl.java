...
import com.nlquery.SqlConversionResult;
...
public class NLQueryServiceImpl implements NLQueryService {
    ...
    public NLQueryResponse executeQuery(NLQueryRequest request) {
        ...
        try {
            // 将自然语言转换为SQL
            SqlConversionResult conversionResult = nlToSqlConverter.convertToSql(request.getQuery(), context);
            ...
            // 记录查询历史
            QueryHistory history = saveQueryHistory(request, conversionResult, results.size(), null);
            
            // 构建响应
            return buildQueryResponse(history, results, conversionResult, request);
    ...
    @Override
    @Transactional
    public SavedQueryDTO saveQuery(String queryId, SavedQueryDTO savedQueryDTO) {
        ...
        savedQuery.setCreatedById("system"); // 应该从当前用户上下文获取
        savedQuery.setCreatedByName("系统用户"); // 应该从当前用户上下文获取
        ...
        return convertToSavedQueryDTO(saved);
    }
    ...
    private NLQueryResponse buildQueryResponse(QueryHistory history, List<Map<String, Object>> results,
        ...
        response.setConfidenceScore(conversionResult.getConfidenceScore());
    ...
    private SavedQueryDTO convertToSavedQueryDTO(SavedQuery savedQuery) {
        ...
        dto.setCreatedById(savedQuery.getCreatedById());
        dto.setCreatedByName(savedQuery.getCreatedByName());
